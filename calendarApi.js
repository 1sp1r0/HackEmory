...
exports.addEvent = function(emails, text) {
  ...
  fs.readFile('client_secret.json', function processClientSecrets(err, content) {
    ...
    // Google Calendar API.
    console.log(text);
    authorize(JSON.parse(content), addEvent, emails, text);
  });
}
 ...
 */
function authorize(credentials, callback, emails, text) {
  ...
  fs.readFile(TOKEN_PATH, function(err, token) {
    if (err) {
      ...
      callback(oauth2Client, emails, text);
    }
  });
}
...
function addEvent(auth, emails, text) {
  var calendar = google.calendar('v3');
  var event = {
    'summary': text,
    'location': 'his bed',
